...
- (void)commonInit {
    ...
    // instance not property so no self. apparently
...
- (void)layoutSubviews {
    ...
    if ([_cache readTimeRange:_timeRange width:size.width error:&error]) {
        ...
        //
       timePerPixel = CMTimeMultiplyByFloat64(_timeRange.duration, 1 / size.width); //might need
        //CMTime timePerPixel = CMTimeMultiplyByFloat64(_timeRange.duration, 1 / size.width); //might need
